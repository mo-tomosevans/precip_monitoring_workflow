...
def is_datetime_instance(instance: object) -> bool:
    """Check if instance is a type of datetime.

    Checks if the object is either
      * a python built in datetime.datetime instance
      * a cftime.datetime instance

    Args:
        instance (object): The object to be checked.

    Returns:
        bool: True if the object is a datetime instance, False otherwise.
    """

    return isinstance(instance, (datetime.datetime, cftime_datetime))
...
def plot_all_PP_plot_handlers(
    ...
    cubes = loader(
        all_input_files,
        iris.AttributeConstraint(STASH=lambda s: s in stash),
    ).load()
...
def plot_all_MetDB_plot_handlers(
    ...
    cubes = iris.load(
        [str(input_dir / f) for f in input_files],
    )
...
class CrudePerformanceTimer:
    """A simple class for timing code execution.

    This timer measures the time between calls to start, lap, and stop.
    It prints the time to stdout when called.
    """
    ...
    def __init__(self, name: str = "UNNAMED") -> None:
        ...
        self._name = name
        self.__perf_counter = perf_counter
        self._start_time = None
        self._last_time = None

    def _print(self, prefix: str, point: str = "") -> None:
        curframe = inspect.currentframe()
        calframe = inspect.getouterframes(curframe, 2)[2]
        caller = f"{calframe.function}-L{calframe.lineno}-{calframe.filename}"

        print(
            f"{self._name} timer: {prefix} ("
            + (f"{point}: " if point else "")
            + f"{caller})"
        )

    def start(self, point: str = "") -> None:
        """Start the timer."""
        if self._start_time:
            return self.lap(point)
        self._start_time = self.__perf_counter()
        self._last_time = self._start_time
        self._print(f"Starting", point)

    def _get_elapsed_time(self, current_time: float) -> datetime.timedelta:
        ...
        return elapsed_timedelta, total_timedelta

    def lap(self, point: str = "") -> None:
        """Get the time since the last lap or start."""
        if self._start_time is None:
            return self.start(point)
        current_time = self.__perf_counter()
        elapsed_time, total_time = self._get_elapsed_time(current_time)
        self._last_time = current_time
        self._print(f"Lap times: total {total_time} lap {elapsed_time}", point)

    def stop(self, point: str = "") -> None:
        """Stop the timer."""
        current_time = self.__perf_counter()
        elapsed_time, total_time = self._get_elapsed_time(current_time)

        self._print(f"Stopping, times: total {total_time} lap {elapsed_time}", point)
        self._start_time = None
        self._last_time = None


# Enable this for debugging:
# global_performance_timer = CrudePerformanceTimer("Global Timer")
