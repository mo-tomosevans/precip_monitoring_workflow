...
def element_from_name(element: ElementBase | str, subtype: str = None) -> ElementBase:
    """Construct an Element object from a name or Element object

    If element is already an instance of ElementBase it will be returned
    unmodified, otherwise the element will be constructed from the settings
    in the config file.


    Args:
        element (ElementBase | str): A pre-constructed element or
                                     the name of an element as specified
                                     in the config file.
        subtype (str): The MetDB subtype of the element. Not required if
                       element is an instance of ElementBase.

    Returns: Initialized Element object
    """
    ...
    if subtype is None:
        raise ValueError(
            "element_from_name: subtype must be provided if element is a string"
        )

    element_name = element.get("element_name", None)
    ...
    return ElementType(subtype=subtype, **element_kwargs)
...
class CompoundElement(SimpleUniqueIDElementMixin, ElementBase):
    """Class that constructs an element from multiple other elements

        The element is constructed by calling the "value_calculation_callback"
        with the observation as its only argument. The callback should return
        the value to be used as the observation value.

    .. note::
       The elements must all be from the same MetDB subtype
    """

    def __init__(self, **kwargs) -> None:
        self._component_elements: list[ElementBase] = [
            element_from_name(el, subtype=kwargs.get("subtype", None))
            for el in kwargs.pop("component_elements")
        ]
        super().__init__(**kwargs)

        _tmp_element_names = set()
        for c in self._component_elements:
            _tmp_element_names = _tmp_element_names | set(c._metdb_element_names)
        self._metdb_element_names = list(_tmp_element_names)

    def get_time_cell(self, ob):
        return (self.element_dict["timestamp_callback"](ob), None)

    def valid_ob(self, ob) -> bool:
        """Validate an observation for each of the component elements and their combination

        The observation will be checked against each of the component elements. It will
        be rejected unless it is valid for all of them.

        To validate the combination of the components the class must be initialized
        with the "component_validation_callback" set to a callable that takes two
        arguments, an observation and a list of the component elements. It should
        return true if a valid compound observation can be constructed.
        """
        for el in self._component_elements:
            if not el.valid_ob(ob):
                return False

        if "component_validation_callback" in self:
            return self["component_validation_callback"](ob, self._component_elements)

        return True

    def configure_units(self, units_dict: dict):
        for el in self._component_elements:
            el.configure_units(units_dict)
        super().configure_units(units_dict)

    def get_element_value_from_ob(self, ob):
        """Uses "value_calculation_callback to calculate the value of the compound observation

        The "value_calculation_callback" should be a callable that takes an observation as
        its only argument.
        """
        return self["value_calculation_callback"](ob)

    def get_extra_static_coords(self, *args, **kwargs) -> list:
        """Get extra iris coordinates that are common to all instances of this element

        Uses the "extra_static_coords_callback" to get the extra coordinates that
        are required to describe this element.

        The callback will be passed this element (i.e 'self') as its first argument
        and any additional arguments passed to this method.
        """

        if "extra_static_coords_callback" in self:
            return self["extra_static_coords"](self, *args, **kwargs)
...
class ObsBase(abc.ABC):
    ...
    def __init_subclass__(cls, **kwargs) -> None:
        ...
        # ensure class has required read only properties set

        required_attributes = [("REQUIRED_ELEMENT_KEYS", list), ("_subtype", str)]

        if isabstract(cls):
            # Abstract Base Classes can't be used directly
            # so they don't need the required properties
            return

        for required, required_type in required_attributes:

            if not hasattr(cls, required):
                raise NotImplementedError(
                    f"{cls.__name__} must have a {required} attribute"
                )
            ...
            if not isinstance(val, property):
                raise TypeError(
                    f"{cls.__name__} {required} attribute must be a property"
                )

            if not isinstance(val.fget(cls), required_type):
                raise TypeError(
                    f"{cls.__name__} {required} property getter must return type {required_type}"
                )

            if val.fset:
                raise ValueError(
                    f"{cls.__name__} {required} property must not define a setter"
                )
...
class SiteObsABC(ObsBase, abc.ABC):
    ...
    def get_obs(self, start_time, end_time, element_dicts: list) -> list:
        ...
        for element in element_dicts:
            ...
            element = element_from_name(element, subtype=self._subtype)
    ...
    def _ob_to_timestamp(self, ob):
        return (
            ...
            / float(SECONDS_IN_HOUR)
        )

    def _is_valid_obset(self, ob) -> bool:
        for element in self._common_elements.values():
            if element not in ob:
                return False
            if np.ma.is_masked(ob[element]) and ob[element].mask.all():
                return False
        return True
        ...
        for element in elements:
            ...
            for ob in observations:
                if not element.valid_ob(ob):
                    continue
                if not self._is_valid_obset(ob):
                    continue
...
class LNDSYNObs(SiteObsABC):
    ...
    @property
    def REQUIRED_ELEMENT_KEYS(self):
        return ["common_name", "element_name"]
    ...
    def _is_valid_obset(self, ob) -> bool:
        for element in ["WMO_STTN_NMBR", "WMO_BLCK_NMBR"]:
            if element not in ob:
                return False
            if np.ma.is_masked(ob[element]) and ob[element].mask.all():
                return False
        return super()._is_valid_obset(ob)
...
class RainfallObs(SiteObsABC):
    """Class for retrieving MetDB RAINFALL subtypes"""
    ...
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self._common_elements["%ID"] = "SITE_ID"
    ...
    # # TODO: use SiteObsABS.cubes_from_obs below??
    # def cubes_from_obs(self, elements, observations, cube_postproc_callback):




    #     return super().cubes_from_obs(elements, observations, cube_postproc_callback)
