...
try:
    ...
    from moxie.plot.text import LATEX_UNITS as MOX_LATEX_UNITS
except ImportError:
    LOGGER.warning(
...
class Colorbar:
    ...
    @property
    def moxie_unit_label(self) -> str:
        "The unit in a moxie friendly format"

        # TODO review this once https://github.com/MetOffice/moxie/issues/155
        #      has been resolved

        if self.unit_label:
            sub_units = str(self.unit_label).strip().split()

            for unit in sub_units:
                if unit not in MOX_LATEX_UNITS:
                    # Ensure moxie recognizes the string as a unit
                    # so that it doesn't apply automatic formatting
                    MOX_LATEX_UNITS[unit] = unit

            # moxie uses spaces to indicate different labels
            # when it plots multiple colorbars at once. oemplotlib
            # doesn't support this, so replace the spaces with underscores
            # which moxie will convert to spaces
            moxified = "_".join(sub_units)

            # moxie always capitalizes the first character in a string so
            # put the units in brackets
            if not (moxified.startswith("(") and moxified.endswith(")")):
                moxified = f"({moxified})"
            return moxified
...
def _make_config_class(cls_name):
    ...
    colormap_input = settings.get("colormap", None)
    ...
    if colormap_input is None:
        for p in parent_classes:
            if issubclass(p, Colorbar) and p is not Colorbar:
                break
        else:
            raise ValueError(
                f"Colormap required for {cls_name} as it does not inherit from another Colorbar"
            )
        default_cmap = None
    elif isinstance(colormap_input, str):
    ...
    # Dynamically create the colorbar class
    TmpCbar = type(cls_name, tuple(parent_classes), {})
