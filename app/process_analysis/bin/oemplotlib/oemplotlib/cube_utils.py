...
def accum_sub_hrly_to_hr(
    cubes, interval_mins, time_coord=DEFAULT_CUBE_TIME_COORD, period=1, RAINFALL=False
):

    time_slices = iris.cube.CubeList(cubes.slices_over(time_coord))
    no_of_slices = len(time_slices)

    assert (
        cubes.coord(time_coord).points > 1,
        AssertionError("cube needs more than 1 time coordinate to be accumulated"),
    )

    # assert interval minutes
    interval_mins_over_hr = interval_mins / 60
    timeslice_interval = (
        time_slices[1].coord(time_coord).points
        - time_slices[0].coord(time_coord).points
    )
    assert (
        timeslice_interval == interval_mins_over_hr,
        AssertionError("incorrect interval given, ensure this is given in minutes"),
    )

    if RAINFALL is not False:
        # If T is cycle time, RAINFALL retrieves T-1H, T-45m, T-30m, T-14m and T. Remove T-1H for correct accumulations.
        time_slices.pop(0)
        assert len(time_slices) == no_of_slices - 1, AssertionError(
            "time slice not removed"
        )
        # latest timeslice should be current cycle
        latest_cubeslice = time_slices[-1]
        latest_time = latest_cubeslice.coord("time")
        latest_time_points = latest_time.points
        latest_time_in_dt_format = latest_time.units.num2date(latest_time_points)
        for time_point in latest_time_in_dt_format:
            assert time_point.minute == 0 and time_point.second == 0, AssertionError(
                f"latest time coordinate {time_point} is not on the hour"
            )

    summed_data = np.nansum([sl.data for sl in time_slices], axis=0)
    assert summed_data.ndim == 1, AssertionError("summed data is not 1D")

    summed_cube = latest_cubeslice.copy(data=summed_data)
