...
class _AutoCbarMixin:
    ...
    def _make_oem_kwargs(self, attrs: Union[List[str], List[tuple]] = None) -> dict:
        ...
        Args:
            attrs (List of str or tuple, optional): List of attributes to be extracted
                                                    from the colorbar. It the attribute
                                                    is a string it must be the name of an
                                                    attribute of the colorbar.
                                                    If the attribute is a 2 tuple the first
                                                    value should be the key to be added to the
                                                    oem keyword dict and the second value should
                                                    be the name of the attribute of the colorbar
                                                    to look up.
                                                    Defaults to None.
        ...
        if self._oem_cbar is None or attrs is None:
            # nothing to do
            return oem_kwargs
        ...
        for attr in attrs:
            if isinstance(attr, tuple):
                ...
                kwarg_key = cbar_attr_name = attr
            cbar_attr = getattr(self._oem_cbar, cbar_attr_name, None)
            if cbar_attr:
    ...
    def pcolormesh(self, cube, layer="default", **kwargs):
        ...
        oem_kwargs = self._make_oem_kwargs(["cmap", ("levels", "bounds")])
        oem_kwargs.update(kwargs)
    ...
    def colorbar(self, layer=None, **kwargs):
        ...
        oem_kwargs = self._make_oem_kwargs(
            [("boundaries", "bounds"), ("label", "unit"), "values", "extend"]
        )
        oem_kwargs.update(kwargs)
        tmp_var = super().colorbar(layer=layer, **oem_kwargs)
