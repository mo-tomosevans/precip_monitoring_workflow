...
def nimrod_to_cubes(
    ...
    constraint=iris.Constraint applied to each cube as it is loaded.
    filterfunc=Function applied to each cube as it is loaded. Function must take an iris cube as its sole argument and return the same.
        Example using select_cubes: filterfunc=lambda cube: n2c.select_cubes(iris.cube.CubeList([cube]), field_code=63).merge_cube()
    ...
    header_data = HeaderInfo(
        filename, quiet=quiet, ignoreErrors=ignoreErrors, debug=debug
    )
        ...
        while True:
            try:
                ...
                if nimrodobj.vt_year <= 0:
                    ...
                    valid_time = dt.datetime(2016, 1, 1, 0, 0, 0)
                else:
                ...
                if printheader:
                    print(
                        count, valid_time, data_time, header_data.averagingtype[count]
                    )
                ...
                if abs(central_meridian_sf - ng_central_meridian_sf_dp) < 1.0e-04:
                    # Set NG scale factor to double precision accuracy
                    central_meridian_sf = ng_central_meridian_sf_dp
                ...
                x_coord, y_coord = _add_horizontal_coords(
                    nx,
                    ny,
                    xorigin,
                    yorigin,
                ...
                if metaonly:
                    ...
                    if header_data.datatype[count] == 1:
                        thisdata = np.ma.masked_equal(
                            nimrodobj.data, header_data.imdi[count]
                        )
                    elif header_data.datatype[count] == 0:
                        thisdata = np.ma.masked_inside(
                            nimrodobj.data,
                ...
                try:
                    ...
                    raise NimrodException("Units not recognised by Iris: " + unitstring)
                ...
                try:
                    ...
                    raise NimrodException(
                        """Unable to add time coordinates for time {} and
                                             forecast_reference_time
                                             {}""".format(
                            valid_time, data_time
                        )
                    )
                ...
                try:
                    ...
                    raise NimrodException(
                        """Height metadata issue, unable to add height
                                             coordinate to cube, height_type:{}, height value:
                                             {}""".format(
                            height_type, height_val
                        )
                    )
                ...
                try:
                    ...
                    if (
                        rematcher.match(thissource) is not None
                        or thissource.find("umek") == 0
                    ):
                ...
                multimemberfield = _add_probability_info(
                    cube, header_data, count, metaonly=metaonly
                )
                ...
                if nimrodobj.ensemble_member >= 0 and not multimemberfield:
                    cube.add_aux_coord(
                        iris.coords.AuxCoord(
                            nimrodobj.ensemble_member,
                            standard_name="realization",
                            units=None,
                        )
                    )
                if nimrodobj.ensemble_member == -98 or any(
                    [
                        True
                        for x in header_data.chead[count].split(" ")
                        if x.find("Mean") >= 0
                    ]
                ):
                ...
                if nimrodobj.ensemble_member == -99 or any(
                    [
                        True
                        for x in header_data.chead[count].split(" ")
                        if x.find("Spread") >= 0
                ...
                cube.attributes.update({"field_code": header_data.field_numbers[count]})
                ...
                if constraint is not None:
                    ...
                    if not quiet and cube is not None:
                        print("Found Cube after constraint:")
                        print(cube)
                # Apply filter function:
                if filterfunc is not None:
                    cube = filterfunc(cube)
                    if isinstance(cube, iris.cube.CubeList):
                        if len(cube) > 0:
                            ...
                            cube = None
                    if not quiet and cube is not None:
                        print("Found Cube after filterfunc:")
                        print(cube)
...
def _add_horizontal_coords(
    ...
    rmdi = (-32767.0, float(idmi))

    if grid == 0 or grid == 4:
        if (
            np.any([lat_true_orig == v for v in rmdi]) and grid == 0
        ):  # some Radarnet files miss
            lat_true_orig = 49.0  # this, number from OSGB definition
        if (
            np.any([long_true_orig == v for v in rmdi]) and grid == 0
        ):  # some Radarnet files miss
            long_true_orig = -2.0  # this, number from OSGB definition
        if (
            np.any([easting_true_orig == v for v in rmdi]) and grid == 0
        ):  # some Radarnet files miss
            easting_true_orig = 400000.0  # this, number from OSGB definition
        if (
            np.any([northing_true_orig == v for v in rmdi]) and grid == 0
        ):  # some Radarnet files miss
            northing_true_orig = -100000.0  # this, number from OSGB definition
        if (
            np.any([central_meridian_sf == v for v in rmdi]) and grid == 0
        ):  # some Radarnet files miss
            central_meridian_sf = 0.9996012717  # this, number from OSGB definition
        if (
            np.any([central_meridian_sf == v for v in rmdi]) and grid == 4
        ):  # some e3195 files miss
        ...
        raise NimrodException(
            "Unsupported grid type, only NG, EuroPP and lat/long are possible"
        )
...
def _get_ellipsoid(ellipsoid, grid, idmi):
    ...
    if ellipsoid == 0:
        ...
        raise NimrodException(
            "Ellipsoid not supported, ellipsoid:{}, grid:{}".format(ellipsoid, grid)
        )
    return ellipsoid
...
def _updatetitle(nimrodobj, header_data, count):
    ...
    thresh_sn = None
    if (
        header_data.field_numbers[count] == 161
        and header_data.thresh_value[count] >= 0.0
    ):
    ...
    if (
        header_data.field_numbers[count] == 29
        and header_data.thresh_value[count] >= 0.0
    ):
    ...
    if header_data.field_numbers[count] == 804 and header_data.vert_value[count] >= 0.0:
        nimrodobj.title = "wind speed"
        thresh_sn = None
    if header_data.field_numbers[count] == 806 and header_data.vert_value[count] >= 0.0:
    ...
    if thresh_sn is None:
        ...
        return (
            nimobjloc,
            iris.coords.AuxCoord(
                thresh_value, standard_name=thresh_sn, units=thresh_units
            ),
        )
...
def _add_time_coords(
    ...
    if debug:
        print(
            "Attempting to add bounds starting {} {} before validity-time.".format(
                bound_length, bound_units
            )
        )
    if bound_length > 0:
        if (
            bound_units is "minutes"
            or bound_units is "minute"
            or bound_units is "mins"
            or bound_units is "min"
        ):
            lb_delta = dt.timedelta(minutes=bound_length)
        if (
            bound_units is "hours"
            or bound_units is "hour"
            or bound_units is "hrs"
            or bound_units is "hr"
        ):
            lb_delta = dt.timedelta(hours=bound_length)
        if bound_units is "seconds" or bound_units is "second" or bound_units is "sec":
    ...
    cube.add_aux_coord(
        iris.coords.AuxCoord(
            time_value_num, standard_name="time", bounds=bounds, units=time_unit
        )
    )
    cube.add_aux_coord(
        iris.coords.AuxCoord(
            forecast_reference_time_num,
    ...
    cube.add_aux_coord(
        iris.coords.AuxCoord(
            seconds, standard_name="forecast_period", bounds=bounds, units=u
        )
    )
...
def _get_averaging(num, period=None):
    ...
    cell_methods = []
    attributes = []
    time_averaging_dictionary = {
        8192: {
            "cf_name": "maximum",
            "cf_coord": "time",
            "long_name": "maximum in period",
            "cell": True,
        },
        4096: {
            "cf_name": "minimum",
            "cf_coord": "time",
            "long_name": "minimum in period",
            "cell": True,
        },
        ...
        128: {
            "cf_name": "mean",
            "cf_coord": "time",
            "long_name": "accumulation or average",
            "cell": True,
        },
...
def _add_probability_info(cube, header_data, count, metaonly=False, debug=False):
    ...
    try:
        ...
        if debug:
            print(
                """Not a probability field as thresh_type ({}) not
                  matched""".format(
                    header_data.thresh_type[count]
                )
            )
    ...
    if header_data.thresh_type[count] == 3:
        ...
        try:
            ...
            if debug:
                print(
                    "No default units for field code {}".format(
                        header_data.field_numbers[count]
                    )
                )
    thisval = None
    if header_data.thresh_value_alt[count] > -32766.0:
        thisval = header_data.thresh_value_alt[count]
    elif header_data.thresh_value[count] > -32766.0:
    ...
    if thisval is not None:
        if header_data.prob_fuzzyth[count] > -32766.0:
            bounds = [
                thisval * header_data.prob_fuzzyth[count],
                thisval * (2.0 - header_data.prob_fuzzyth[count]),
            ]
        else:
            bounds = None
        new_coord = iris.coords.AuxCoord(
            thisval, standard_name=None, long_name=probname, units=units, bounds=bounds
        )
        if debug:
        ...
        if debug:
            print(
                """Not a probability field as thresh_value ({}) and thresh_value_alt ({}) not
                  valid""".format(
                    header_data.thresh_value[count], header_data.thresh_value_alt[count]
                )
            )
    ...
    if header_data.prob_neighradius[count] > 0:
        neigh_radius = "{} {}".format(
            header_data.prob_neighradius[count], cf_units.Unit("km")
        )
        if (
            header_data.prob_niters[count] >= 0
            and 0.0 <= header_data.prob_rfalpha[count] <= 1.0
        ):
            commentstr = """Neighbourhood + {}.RecursiveFilter ({})""".format(
                header_data.prob_niters[count], header_data.prob_rfalpha[count]
            )
        else:
            commentstr = "Neighbourhood"
        cube.add_cell_method(
            iris.coords.CellMethod(
                "mean", coords="area", comments=commentstr, intervals=neigh_radius
            )
        )
    if header_data.prob_radius[count] > 0:
        radius = "{} {}".format(header_data.prob_radius[count], cf_units.Unit("km"))
        commentstr = None
        cube.add_cell_method(
            iris.coords.CellMethod(
                "mean", coords="area", comments=commentstr, intervals=radius
            )
        )
    attributes = []
    ...
    if header_data.prob_threshwindow[count] >= 0:
        cube.attributes["Probability methods"].append(
            """minimum_time {} minutes""".format(header_data.prob_threshwindow[count])
        )
    if debug:
...
def _add_vertical_coord(cube, height_type, height_val, height_val_upper):
    ...
    Args:
     * cube (Cube) - the cube we want to add units to.
     * height_type (integer) - the header entry 20 in the Nimrod file
    ...
    if height_val_upper == 9999.0 and height_val == 9999.0:
        height_val_upper = 0.0  # A bit nonsensical.
    if height_val == 9999.0 or height_val == 8888.0:
        height_val = 0.0  # Relies on correct vert_coord_type instead.

    if height_val_upper >= 0.0 and height_val_upper != height_val:
...
def select_cubes(cubelist, nofail=False, **kwargs):
    """
    ...
    nofail=boolean: Set to True to avoid raising IndexError if no matching cubes are found.
    ...
    if "field_code" in kwargs:
        ...
        constraints = constraints & iris.AttributeConstraint(field_code=fieldcode)
    ...
    if "averaging_period" in kwargs:
        ...
        constraints = constraints & iris.Constraint(
            cube_func=lambda cube: averaging_period
            in [x.intervals[0] for x in cube.cell_methods]
        )
    if "experiment_no" in kwargs:
        exp_no = kwargs["experiment_no"]
        constraints = constraints & iris.AttributeConstraint(experiment_number=exp_no)
    if "attribute" in kwargs:
        attribute_name = kwargs["attribute"]
        if isinstance(attribute_name, str):
            attribute_name = [
                attribute_name
            ]  # Avoids string being handled char-by-char
        constraints = constraints & iris.AttributeConstraint(processing=attribute_name)
    ...
    if len(output) == 0:
        if nofail:
            warnings.warn("No cubes with that specification found ({})".format(kwargs))
        else:
            raise IndexError(
                "No cubes with that specification found ({})".format(kwargs)
            )
    if len(output) > 1:
        warnings.warn("More than one cube matching search critera is formed")
    return output
        ...
        with open(filename, "rb") as f:
            ...
            while initial_pos < filesize:
                try:
                    ...
                    if _pad1[0] != 512:
                        raise NimrodException(
                            """Nimrod header seems not to be of size 512.
                    ...
                    if self.averaging_period != 32767:
                        ...
                        _unpack_data(
                            f, "field_pos", initial_pos, 2, ">h", self.averaging_period
                        )
                        self.averaging_period_units.append("secs")

                    _unpack_data(
                        f, "field_pos", initial_pos, 2, ">h", (self.field_numbers)
                    )
                    _unpack_data(
                        f,
                        "vert_coord_type_pos",
                    ...
                    _unpack_data(
                        f, "vert_value_pos", initial_pos, 4, ">f", (self.vert_value)
                    )
                    _unpack_data(
                        f,
                        "vertupper_coord_type_pos",
                        initial_pos,
                        2,
                        ">h",
                        (self.vertupper_coord_type),
                    ...
                    _unpack_data(
                        f, "averaging_pos", initial_pos, 2, ">h", (self.averagingtype)
                    )
                    _unpack_data(
                        f, "yorigin_pos", initial_pos, 4, ">f", (self.y_origins)
                    )
                    _unpack_data(f, "yresol_pos", initial_pos, 4, ">f", (self.y_resol))
                    _unpack_data(
                        f, "xorigin_pos", initial_pos, 4, ">f", (self.x_origins)
                    )
                    _unpack_data(f, "xresol_pos", initial_pos, 4, ">f", (self.x_resol))
                    _unpack_data(
                        f,
                        "unit_scalefactor_pos",
                        initial_pos,
                        4,
                        ">f",
                        (self.unit_scalefactor),
                    ...
                    _unpack_data(
                        f, "thresh_type_pos", initial_pos, 2, ">h", (self.thresh_type)
                    )
                    _unpack_data(
                        f, "prob_method_pos", initial_pos, 2, ">h", (self.prob_method)
                    )
                    _unpack_data(
                        f, "prob_niters_pos", initial_pos, 2, ">h", (self.prob_niters)
                    )
                    _unpack_data(
                        f,
                        "prob_nmembers_pos",
                    ...
                    _unpack_data(
                        f, "prob_radius_pos", initial_pos, 4, ">f", (self.prob_radius)
                    )
                    _unpack_data(
                        f, "prob_rfalpha_pos", initial_pos, 4, ">f", (self.prob_rfalpha)
                    )
                    _unpack_data(
                        f, "prob_fuzzyth_pos", initial_pos, 4, ">f", (self.prob_fuzzyth)
                    )
                    _unpack_data(
                        f,
                        "prob_fuzzytime_pos",
                    ...
                    if debug:
                        print(
                            """Finished reading field {}. Next field starts at byte {}.
                              """.format(
                                cnt, initial_pos
                            )
                        )
...
def main(argv):
    ...
    infile = argv[0]
    outfile = argv[1]
    print(" Reading following Nimrod file into cubes: %s" % (infile))
    cubes = nimrod_to_cubes(infile, quiet=True)
