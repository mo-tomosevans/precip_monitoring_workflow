...
class OEMRegridderABC(ABC):
    def __init__(
        ...
        # TODO: This line works around a cartopy bug which drops the rotation
        #       of the rotated pole when copying coordinate systems
        #       We'll set up a handler using the original target grid to avoid copying
        #       and we can reference that elsewhere in the code.
        #       This has been fixed in newer versions of cartopy.
...
class NullRegridder(OEMRegridderABC):
    ...
    def __init__(
        self, source_grid: iris.cube.Cube, target_grid_name: str = None,
    ) -> None:
        """
        Set up...
...
class MultiStepRegridder(OEMRegridderABC):
    ...
    def __init__(
        ...
        super().__init__(
            source_grid, target_grid, None, target_grid_name,
        )
